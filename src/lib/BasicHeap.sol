...
library BasicHeap {
    ...
    struct Account {
        ...
        int256 value; // The value of the account.
    }
    ...
    /// @return The list of Accounts for the heap
    function accountList(Heap storage heap) internal view returns (Account[] memory) {
        return heap.accounts;
    }
    ...
    function getValueOf(Heap storage heap, address id) internal view returns (int256) {
    ...
    function getValueOfRoot(Heap storage heap) internal view returns (int256) {
        if (heap.accounts.length > 0) {
            return heap.accounts[_ROOT].value;
        } else {
            return 0;
        }
    }
    ...
    function insert(Heap storage heap, address id, int256 value) internal {
    ...
    function decrease(Heap storage heap, address id, int256 newValue) internal {
        uint256 index = heap.indexOf[id];
    ...
    /// @param newValue The new value of the account.
    function increase(Heap storage heap, address id, int256 newValue) internal {
        uint256 index = heap.indexOf[id];
    ...
    function update(Heap storage heap, address id, int256 value) internal {
        uint256 index = heap.indexOf[id];
        uint256 accountsLength = heap.accounts.length;

        if (_containsAccount(heap, index, accountsLength, id)) {
            int256 previousValue = heap.accounts[index].value;

            if (value > previousValue) {
                // increase
                _shiftUp(heap, Account(id, value), index);
            } else if (value < previousValue) {
                // decrease
                _shiftDown(heap, accountsLength, Account(id, value), index);
            }
        } else {
            // insert
            if (id == address(0)) revert AddressIsZero();
    ...
    function _shiftUp(Heap storage heap, Account memory accountToShift, uint256 index) private {
        int256 valueToShift = accountToShift.value;
