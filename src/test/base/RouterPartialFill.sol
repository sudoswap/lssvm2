 ...
 */
abstract contract RouterPartialFill is Test, ERC721Holder, ConfigurableWithRoyalties, RouterCaller {
    ...
    function test_defaultFullFill() public {
        ...
        for (uint256 numNFTs = 1; numNFTs <= 10; numNFTs++) {
            ...
            LSSVMRouter2.PairSwapSpecificPartialFill[] memory buyList = new LSSVMRouter2.PairSwapSpecificPartialFill[](
                    1
                );
            ...
            uint256[] memory partialFillPrices = router.getNFTQuoteForPartialFillBuy(pair, NUM_NFTS);

            // Create the partial fill args
            buyList[0] = LSSVMRouter2.PairSwapSpecificPartialFill({
                swapInfo: LSSVMRouter2.PairSwapSpecific({pair: pair, nftIds: ids}),
                expectedSpotPrice: SPOT_PRICE,
                maxCostPerNumNFTs: partialFillPrices
            });

            // Create empty sell list
            LSSVMRouter2.PairSwapSpecificPartialFillForToken[] memory emptySellList =
            new LSSVMRouter2.PairSwapSpecificPartialFillForToken[](
                    0
                );
    ...
    function test_restrictedPartialFill(uint64 delta) public {
        ...
        for (uint256 numNFTsToBuyFirst = 1; numNFTsToBuyFirst <= 9; numNFTsToBuyFirst++) {
            ...
            uint128 newDelta = uint128(pair.delta() * numNFTsToBuyFirst * uint256(block.timestamp));
            ...
            // Get partial fill prices
            uint256[] memory partialFillPrices = router.getNFTQuoteForPartialFillBuy(pair, 10);
            // Create the partial fill args
            buyList[0] = LSSVMRouter2.PairSwapSpecificPartialFill({
                swapInfo: LSSVMRouter2.PairSwapSpecific({pair: pair, nftIds: ids}),
                expectedSpotPrice: SPOT_PRICE,
            ...
            (,,, uint256 initialQuote,) = pair.getBuyNFTQuote(numNFTsToBuyFirst);
            LSSVMRouter2.RobustPairSwapSpecific[] memory initialBuyList = new LSSVMRouter2.RobustPairSwapSpecific[](
                    1
                );
            initialBuyList[0] = LSSVMRouter2.RobustPairSwapSpecific({
                swapInfo: LSSVMRouter2.PairSwapSpecific({pair: pair, nftIds: nftIdsToBuyFirst}),
                maxCost: initialQuote
            });
            // Buy these items first (if we can)
            pair.swapTokenForSpecificNFTs{value: this.modifyInputAmount(initialQuote)}(
    ...
    function test_partialFill(uint64 delta) public {
        ...
        for (uint256 numNFTsToBuyFirst = 1; numNFTsToBuyFirst <= 9; numNFTsToBuyFirst++) {
            ...
            (, uint256 newSpotPrice,,,) = pair.getBuyNFTQuote(numNFTsToBuyFirst);
