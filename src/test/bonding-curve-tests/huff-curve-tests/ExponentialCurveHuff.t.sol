...
contract ExponentialCurveHuffTest is Test {
    ...
    ExponentialCurve public curve;

    function setUp() public {
        curve = ExponentialCurve(HuffDeployer.deploy("bonding-curves/huff-curves/ExponentialCurve"));
    }

    function test_validateDelta() public {
        assertTrue(curve.validateDelta(2 ether));
        assertFalse(curve.validateDelta(1 ether));
    }

    function test_validateSpotPrice() public {
        assertTrue(curve.validateSpotPrice(0xF4240));
