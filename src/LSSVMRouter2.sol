...
contract LSSVMRouter2 {
    ...
    function getNFTQuoteForPartialFillBuy(LSSVMPair pair, uint256 numNFTs) external view returns (uint256[] memory) {
        ...
        // Locally scope the buys
        {
            ...
            for (uint256 i; i < numBuys;) {
                ...
                if (spotPrice <= buyList[i].expectedSpotPrice) {
                    ...
                    remainingValue -= pair.swapTokenForSpecificNFTs{value: buyList[i].maxCostPerNumNFTs[numNFTs - 1]}(
                ...
                // If spot price is is greater, then potentially 1 or more items have already been bought
                    ...
                    (uint256 numItemsToFill, uint256 priceToFillAt) =
                        _findMaxFillableAmtForBuy(pair, numNFTs, buyList[i].maxCostPerNumNFTs);
                    ...
                    if (numItemsToFill == 0) {
                        ...
                        uint256[] memory fillableIds =
                            _findAvailableIds(pair, numItemsToFill, buyList[i].swapInfo.nftIds);
                        ...
                        remainingValue -= pair.swapTokenForSpecificNFTs{value: priceToFillAt}(
        ...
        // Locally scope the sells
        {
            ...
            for (uint256 i; i < sellList.length;) {
                ...
                // Otherwise, run partial fill calculations
                    ...
                    (uint256 numItemsToFill, uint256 priceToFillAt) =
                        _findMaxFillableAmtForETHSell(pair, numNFTs, sellList[i].minOutputPerNumNFTs);
                    pair.swapNFTsForToken(
     ...
     */
    function _findMaxFillableAmtForBuy(LSSVMPair pair, uint256 maxNumNFTs, uint256[] memory maxPricesPerNumNFTs)
        internal
        view
    ...
    function _findMaxFillableAmtForETHSell(LSSVMPair pair, uint256 maxNumNFTs, uint256[] memory minOutputPerNumNFTs)
        internal
        view
        returns (uint256 numNFTs, uint256 price)
    {
     ...
     */
    function _findAvailableIds(LSSVMPair pair, uint256 numNFTs, uint256[] memory potentialIds)
        internal
        view
    ...
    function buyNFTsThenSellWithETH(
        ...
        // Locally scope the buys
        {
            ...
            for (uint256 i; i < numBuys;) {
                // Total ETH taken from sender cannot msg.value
                // because otherwise the deduction from remainingValue will fail
                remainingValue -= buyList[i].swapInfo.pair.swapTokenForSpecificNFTs{value: buyList[i].maxCost}(
        ...
        // Locally scope the sells
        {
            ...
            for (uint256 i; i < numSwaps;) {
                ...
                sellList[i].swapInfo.pair.swapNFTsForToken(
                    sellList[i].swapInfo.nftIds, sellList[i].minOutput, payable(msg.sender), true, msg.sender
                );
        ...
        returns (uint256 remainingValue)
    {
        ...
        for (uint256 i; i < numSwaps;) {
            ...
            // because otherwise the deduction from remainingValue will fail
            remainingValue -= swapList[i].swapInfo.pair.swapTokenForSpecificNFTs{value: swapList[i].maxCost}(
     ...
     */
    function swapNFTsForToken(RobustPairSwapSpecificForToken[] calldata swapList)
        external
        returns (uint256 outputAmount)
    {
        ...
        for (uint256 i; i < numSwaps;) {
            ...
            outputAmount += swapList[i].swapInfo.pair.swapNFTsForToken(
                swapList[i].swapInfo.nftIds, swapList[i].minOutput, payable(msg.sender), true, msg.sender
            );
